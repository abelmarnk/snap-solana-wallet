  ...
  describe('getMultipleTokenConversions', () => {
    ...
    describe('when data is cached', () => {
      it('returns cached data', async () => {
        // Mock some non-undefined data that the cache initially contains
        const cachedData = {};
        jest.spyOn(mockCache, 'get').mockResolvedValue(cachedData);
        ...
        expect(mockPriceApiClient.getMultipleSpotPrices).not.toHaveBeenCalled();
      });
    });
  });
